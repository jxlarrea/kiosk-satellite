...
void main() {
  ...
  test('discovery announces a timestamp sensor without a value yet',
      () async {
    await build();
    final config = link
    ...
    expect(parsed['state_topic'],
        'kiosksatellite/testdev1/last_interaction/state');
    // Unknown until someone actually interacts; the broker's retained copy
    // covers restarts.
    expect(link.published['kiosksatellite/testdev1/last_interaction/state'],
        isNull);
  });

  test('touches and spoken turns stamp the sensor, ambient noise does not',
      () async {
    await build();
