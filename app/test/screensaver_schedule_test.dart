...
void main() {
  group('parseScreensaverSchedule', () {
    ...
    test('an entry without brightness stays without one', () {
      final entries = parseScreensaverSchedule('[{"at":"08:00","mode":"dim"}]');
    ...
    test(
      'the Now Playing override preserves booleans and ignores invalid values',
      () {
        final entries = parseScreensaverSchedule(
          '[{"at":"00:00","mode":"clock","now_playing":true},'
          '{"at":"12:00","mode":"clock","now_playing":false},'
          '{"at":"18:00","mode":"clock","now_playing":"off"},'
          '{"at":"22:00","mode":"clock"}]',
        );
        expect(entries[0]['now_playing'], isTrue);
        expect(entries[1]['now_playing'], isFalse);
        expect(entries[2].containsKey('now_playing'), isFalse);
        expect(entries[3].containsKey('now_playing'), isFalse);
      },
    );
  ...
  group('upsertScheduleEntry', () {
    ...
    test('no index adds a time', () {
      ...
      final next = upsertScheduleEntry(entries, {
        'at': '19:00',
        'mode': 'black',
      }, null);
      expect(next, hasLength(2));
