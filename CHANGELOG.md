...
### Added
- **Show fingers** as a gesture: a hand showing one to four fingers, or an open hand, to the camera, each its own trigger, so five actions can live on one hand. MediaPipe's palm detector proposes the hand and its hand landmark model confirms it and reads the 21 joints from which the extended fingers are counted (a finger counts when nearly straight at its middle joint, the thumb when it points away from the palm rather than across it); the action fires as soon as two of the last three looks agree on the configured count (a hand busy with something else, a vape or a cup at the mouth, reads a wandering count and lands on any number for a single look, which is what the agreement keeps out), and the count has to change, or the hand go, before the same mapping fires again. Only a hand the model is sure of, held with the fingers pointing up, is counted, and a finger counts when it is nearly straight with its tip leading its middle joint away from the wrist. Both models are small, on-device, and run only when the picture changed or a hand was just seen; nothing is identified, stored or compared.
