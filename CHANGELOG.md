...
### Added
- A **volume control on the Now Playing view**: a speaker toggle leads the row of small toggles, and a tap swaps the progress bar for a volume slider with the level beside it, back to the progress bar four seconds after the last touch or on a second tap. It sets the device's media volume for this device's own player, the followed player's own volume for a Music Assistant or Home Assistant player, and the room's or the group's volume for a Sonos. A player without volume control leaves the toggle out, a blank keeping the transport centered.
- **Sonos speakers can be followed directly**, over the speaker's own local interface, with no Home Assistant or Music Assistant in between. The Player picker's Sonos group lists every room of the household as the Sonos app names it, grouped rooms as one entry, found by a search on the tablet's network or added by address for a speaker on another VLAN, which the kiosk then remembers. A followed room shows its track, artwork and progress read from the speaker once a second, so lyrics stay in step, takes play, pause, stop, next, previous, seek and shuffle, lists the speaker's queue in the Now Playing panel with a tap to jump, follows the group the room plays in as the Sonos app regroups it, and stops instead of pausing a radio stream the way the speaker itself does.
- The floating player and the Now Playing view can follow **any Home Assistant media player**, on the device and in the remote admin alike. The **Player** picker at the top of the renamed **Media Player** settings page lists this device, every Music Assistant player and every Home Assistant `media_player` entity by name, and the pick drives the card, the full-screen view and the transport: track, artwork, progress, play and pause, next and previous, seek and shuffle where the player supports them, with the buttons a player cannot honor left out. Every Home Assistant player is treated the same whatever integration stands behind it. Synced lyrics follow a followed player too, timed from the position its own system reports and looked up on LRCLIB when Music Assistant is not set up, and the Now Playing view wears a chip with the followed player's name. A followed Home Assistant player has no queue panel and no favorite heart, since neither exists outside Music Assistant.
- A **Media Player** settings page split into four pages of its own: Sendspin Player (the local player's switch, server, codec, sync offset and voice ducking), Music Assistant (server, token, kiosk menu shortcut), Floating Player (the card's rows) and Now Playing (the full-screen view's rows and the lyrics timing). The Sendspin Player page leaves the settings while another player is followed, as its rows did before.
