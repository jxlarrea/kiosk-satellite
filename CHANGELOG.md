...
- Dashboard view rotation can now crossfade between views (#189). An opt-in Crossfade between views toggle in the Dashboard View Rotation settings, in the device settings and the remote admin alike, makes each rotation step dissolve the current view into the next instead of cutting. The dissolve happens inside the page, riding the same view cache the dashboard carousel uses, so a view the rotation has shown before fades in fully rendered with fresh entity states, while a view's very first pass still switches instantly (nothing rendered exists yet to fade to). Moving to a different dashboard or an external page also still switches instantly, since nothing of the two pages ever coexists to fade between.
