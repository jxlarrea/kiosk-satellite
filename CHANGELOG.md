...
### Fixed
- **A radio station's logo shows on a Sonos followed directly.** A stream's track metadata names the song and carries no art, so the view showed the placeholder; the logo is on the station item the speaker was given to play, which the speaker's media info carries. The player reads it once per stream when the track brings no art of its own, and the cover of a queue track is untouched.
