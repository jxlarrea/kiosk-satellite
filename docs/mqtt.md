...
| App uptime, Network uptime | sensor | Seconds since the app process started, and seconds since the device's network came up, republished once a minute (issue #213). A kiosk that recently restarted or dropped offline shows up as a small number, which is what a status dashboard wants to highlight. Network uptime reads the kernel's own timestamp on the interface's IP address, so it survives app restarts and counts from when the address was configured; a reconnect or a new DHCP address resets it, a lease renewal does not. On a ROM that refuses the kernel read it falls back to a clock that can only start at app start, and then reads as "how long since a drop was last seen" (the app log says which source a device is using). Unknown while the device is offline. |
