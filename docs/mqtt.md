...
| Adaptive brightness | switch | The [Adaptive brightness](screen.md#adaptive-brightness) setting: the device dims its screen from its own ambient light sensor. With it on, the Screen light still reads and writes the panel itself: a write lands as asked, and the room's light scales it from there at its next step. Turn the switch off ahead of a scene that wants the panel exactly where it puts it and kept there. Only exists on devices with a light sensor. |
