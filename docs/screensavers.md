...
The **Widget scaling** slider (50% to 200%) adjusts widget size across the display, offering a live preview while the screensaver is running. When running over the Clock screensaver in Night mode, all active widgets inherit the clock's **Night color** to maintain low light levels. Widgets render over a soft dark vignette to ensure text readability against bright background images. The **Vignette strength** slider (0% to 100%, 80% default) adjusts vignette opacity with live previewing. Setting this to 0% removes shading entirely for a clean frame look.
