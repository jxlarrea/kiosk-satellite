...
Finish the setup wizard on the Portal itself or via a browser at `http://<portal ip>:2324`. No renderer setting is needed: Flutter runs its OpenGL ES backend on the Portal and the app keeps it working across Activity re-creation (detailed below).
...
| Releases up to 2026.9.22 switched the Portal to the Legacy renderer. | Flutter rejects the Portal's Vulkan driver and renders with OpenGL ES. Destroying and re-creating the app's Activity (the Back button in Meta's top bar, then the app tile) left the renderer's context bound to the wrong thread, and from then on the dashboard showed but screensavers, the drawer and settings never drew. Earlier releases worked around it by switching every Portal to Skia on its first start. | Nothing required. The app now releases the context when the Activity is destroyed, so Impeller works on the Portal. A Portal that an earlier release switched to Skia goes back to Impeller by itself on its first start of a fixed build. **Legacy renderer** under **Settings > Device** stays available as a manual choice and is left alone once a person sets it. |
